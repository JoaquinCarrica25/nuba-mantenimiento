...
export default function ApartamentoView({ apartamento }) {
  ...
  const [pestana, setPestana]   = useState("verificacion");
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0">
      ...
      <header className="bg-white border-b border-slate-200 px-4 md:px-8 py-4 sticky top-0 z-20">
        <div className="max-w-4xl mx-auto">
          <div className="flex items-center justify-between gap-4 mb-3">
            <div>
              <h1 className="text-lg md:text-xl font-bold text-nuba-blue tracking-tight">{apartamento}</h1>
              <p className="text-slate-400 text-xs mt-0.5">Hoja de verificación · {total} ítems</p>
            </div>
            {saving && (
              <span className="text-xs text-nuba-cyan font-medium animate-pulse flex items-center gap-1">
                <span className="w-1.5 h-1.5 rounded-full bg-nuba-cyan inline-block"></span>Guardando…
              </span>
            )}
          </div>
            ...
            {[
              ...
              <button key={key} onClick={() => setPestana(key)}
                className={`px-4 py-1.5 rounded-lg text-xs font-semibold transition-all
                  ${pestana === key ? "bg-white text-nuba-blue shadow-sm" : "text-slate-500 hover:text-slate-700"}`}>
                {label}
              </button>
            ))}
          </div>
            ...
            <>
              <div className="mb-3">
                ...
                <div className="h-2 bg-slate-100 rounded-full overflow-hidden">
                  <div className="h-full bg-nuba-cyan rounded-full transition-all duration-500" style={{ width: `${progreso}%` }} />
                </div>
              </div>
              <div className="flex gap-2 text-xs mb-3">
                <span className="bg-nuba-cyan/10 text-nuba-cyan px-2.5 py-1 rounded-full font-semibold">✓ {hechos} comprobados</span>
                <span className="bg-red-50 text-red-500 px-2.5 py-1 rounded-full font-semibold">● {pendientes} pendientes</span>
              </div>
              <div className="flex flex-col sm:flex-row gap-2">
                <div className="flex gap-1.5 bg-slate-100 rounded-xl p-1 flex-shrink-0">
                  {[{ key: "todo", label: "Ver todo" }, { key: "pendientes", label: "Pendientes" }].map(({ key, label }) => (
                    <button key={key} onClick={() => setFiltro(key)}
                      className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all
                        ${filtro === key ? "bg-white text-nuba-blue shadow-sm" : "text-slate-500 hover:text-slate-700"}`}>
                      {label}
                    </button>
                  ))}
                </div>
                <input type="text" placeholder="Buscar ítem u observación…" value={busqueda}
                  onChange={(e) => setBusqueda(e.target.value)}
                  className="flex-1 bg-slate-100 rounded-xl px-3 py-2 text-sm text-slate-700 placeholder-slate-400 outline-none focus:ring-2 focus:ring-nuba-cyan/30 transition" />
              </div>
            </>
          )}
        </div>
      ...
      {/* ── Contenido ── */}
      {pestana === "historial" ? (
        <HistorialView apartamento={apartamento} />
      ) : (
              ...
              <>
                {itemsFiltrados.length === 0 ? (
                  <div className="text-center py-20 text-slate-400">
                    ...
                    <p className="text-sm mt-1">No hay ítems pendientes.</p>
                  </div>
                ) : (
