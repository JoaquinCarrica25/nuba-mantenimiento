// ============================================================
// NUBA MANTENIMIENTO — ItemCard (tarjeta por ítem)
// ============================================================
import { useRef, useState } from "react";
import { FRECUENCIAS, RESPONSABLES } from "../data/data";

export default function ItemCard({ item, onUpdate, onFoto }) {
    ...
    <>
      ...
      >
        ...
        <div className={`px-4 pt-4 pb-3 flex items-start gap-3
          ${item.estado ? "bg-cyan-50/50" : "bg-white"}`}
        >
          {/* Número */}
          ...
          <button
            ...
            aria-label={item.estado ? "Marcar pendiente" : "Marcar comprobado"}
            className={`flex-shrink-0 w-12 h-6 rounded-full relative transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-offset-1
              ${item.estado ? "bg-nuba-cyan focus:ring-nuba-cyan/40" : "bg-slate-300 focus:ring-slate-400"}`}
          >
        ...
        {/* ── Badges: estado + funciona/no funciona ── */}
        <div className="px-4 pb-3 flex items-center gap-2 flex-wrap">

          {/* Badge comprobado/pendiente */}
          ...
          <div className="flex items-center gap-1.5 ml-1">
            ...
            <button
              onClick={() => onUpdate(item.item_id, "funciona", true)}
              className={`px-3 py-1 rounded-full text-xs font-semibold border transition-all duration-150
                ${item.funciona === true
                  ? "bg-nuba-cyan text-white border-nuba-cyan shadow-sm"
                  : "bg-white text-slate-500 border-slate-200 hover:border-nuba-cyan hover:text-nuba-cyan"
                }`}
            >
              ✓ Sí
            </button>
            <button
              onClick={() => onUpdate(item.item_id, "funciona", false)}
              className={`px-3 py-1 rounded-full text-xs font-semibold border transition-all duration-150
                ${item.funciona === false
                  ? "bg-red-500 text-white border-red-500 shadow-sm"
                  : "bg-white text-slate-500 border-slate-200 hover:border-red-400 hover:text-red-400"
                }`}
            >
              ✗ No
            </button>
        ...
        <div className="px-4 pb-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <div className="sm:col-span-2 flex items-center gap-3">
            ...
            {item.foto_url && (
              <button onClick={() => setLightbox(true)} className="relative w-14 h-14 rounded-xl overflow-hidden border-2 border-nuba-cyan/30 hover:border-nuba-cyan transition flex-shrink-0">
